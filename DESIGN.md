...
- 截图优先替换图片内容控件或现有占位图片；否则插入内联图片。保持宽高比，不裁剪，最大宽度为当前节正文宽度，多张图片纵向排列。
...
- 页面数未变化时，对答案区域做遮罩后进行页面图像差异检查。
- 页面数变化时，比较页眉页脚、页面边界、节属性和锚点顺序，并检测空白页、图片越界和缺失边框；不要求整页像素对应。
...
1. 阶段 0：完成设计、领域词汇和 Skill 契约。
2. 阶段 1：实现 DOCX 分析、槽位映射和纯代码填充。
3. 阶段 2：实现截图插入、Word 渲染和保真验证。
4. 阶段 3：接入 PDF 后备模式。
5. 阶段 4：基于真实模板增加表格槽位等扩展能力。
